...
SIDEBAR_NAV_GROUPS = [
    ...
    },
    {
        ...
        "items": [
            ("ai_tutor", "🤖", "AI Tutor"),
            ("ai_mnemonics", "✨", "AI Mnemonics Generator"),
            ("resources", "🧰", "Smart Tools"),
...
def inject_premium_css(theme):
    ...
    st.markdown(
        ...
        [data-testid="stSidebar"] {{
            background:
                radial-gradient(circle at 16% 0%, rgba(139,92,246,0.22), transparent 34%),
                radial-gradient(circle at 90% 18%, rgba(6,182,212,0.16), transparent 32%),
                linear-gradient(180deg, rgba(247,242,255,0.94), rgba(239,232,255,0.82)) !important;
            border-right: 1px solid rgba(139,92,246,0.18);
            box-shadow: 18px 0 54px rgba(36,22,61,0.10);
            backdrop-filter: blur(22px);
        }}
        ...
        .sidebar-brand-card {{
            ...
            background:
                ...
                radial-gradient(circle at 100% 0%, rgba(139,92,246,0.22), transparent 46%);
            padding: 14px;
        ...
        .sidebar-scroll-menu::-webkit-scrollbar-track {{
        ...
        .sidebar-nav-group {{
            ...
            border-radius: 14px;
            background: rgba(255,255,255,0.42);
            box-shadow: 0 10px 26px rgba(36,22,61,0.045);
            margin-bottom: 9px;
            overflow:hidden;
        }}
        ...
        .sidebar-nav-link.active {{
            background: linear-gradient(135deg, #7c3aed, #2563eb 56%, #06b6d4) !important;
        ...
        [data-testid="stSidebar"] [data-testid="stTextInput"] input:focus {{
            ...
            box-shadow: 0 0 0 3px rgba(124,58,237,0.12), 0 10px 26px rgba(31,41,55,0.045) !important;
        }}
...
def render_sidebar():
    ...
    with st.sidebar:
        st.markdown(
            ...
            <div class="sidebar-brand-card">
                ...
                <div class="sidebar-badge-row">
                    <span class="sidebar-exam-badge">SQU-COM</span>
                    <span class="sidebar-exam-badge">OMSB</span>
                    <span class="sidebar-exam-badge">USMLE</span>
                    <span class="sidebar-exam-badge">PLAB</span>
                </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        search_text = st.session_state.get("sidebar_search", "").strip().lower()

        def _visible_nav_items(group):
            items = []
            for page_id, icon, label in group["items"]:
                if search_text and search_text not in label.lower() and search_text not in page_id.lower():
                    continue
                items.append((page_id, icon, label))
            return items

        nav_sections = []
        for group in SIDEBAR_NAV_GROUPS:
            visible_items = _visible_nav_items(group)
            ...
            for page_id, icon, label in visible_items:
                ...
                item_html.append(
                    f'<a class="sidebar-nav-link{active_class}" href="?page={html.escape(page_id)}">'
                    f'<span class="sidebar-link-icon">{html.escape(icon)}</span>'
                    f'<span class="sidebar-link-label">{html.escape(label)}</span>'
                    f'</a>'
                )

            nav_sections.append(
                f'<details class="sidebar-nav-group"{open_attr}>'
                f'<summary><span class="sidebar-group-title">'
                f'<span>{html.escape(group["icon"])}</span>'
                f'<span>{html.escape(group["title"])}</span>'
                f'</span></summary>'
                f'<div class="sidebar-route-list">{"".join(item_html)}</div>'
                f'</details>'
            )

        if not nav_sections:
            nav_sections.append('<div class="sidebar-mini-label">No matching modules</div>')

        st.markdown(
            f'<div class="sidebar-mini-label">Workspace</div>'
            f'<nav class="sidebar-scroll-menu" aria-label="MedStudy navigation">{"".join(nav_sections)}</nav>',
            unsafe_allow_html=True,
        )
